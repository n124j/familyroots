# FamilyRoots — Disaster Recovery Runbook
...
```bash
# 1. Check pod status
kubectl get pods -n familyroots -l app=postgres

# 2. Describe the failing pod
kubectl describe pod <pod-name> -n familyroots

# 3. Check PVC availability
kubectl get pvc -n familyroots

# 4. Force pod restart
kubectl rollout restart deployment/postgres -n familyroots
```
...
kubectl exec -it deploy/postgres -n familyroots -- \
  bash -c "dropdb familyroots && createdb familyroots"

kubectl exec -i deploy/postgres -n familyroots -- \
  pg_restore --dbname=familyroots --no-owner --no-acl /dev/stdin < /tmp/restore.dump

# Run Alembic migrations to bring schema to HEAD
kubectl exec deploy/api-blue -n familyroots -- alembic upgrade head
```
...
```bash
# Restart Redis
kubectl rollout restart deployment/redis -n familyroots

# Verify connectivity
kubectl exec -it deploy/redis -n familyroots -- redis-cli ping
```
...
kubectl get svc api -n familyroots -o jsonpath='{.spec.selector.slot}'

# Roll traffic back to old slot (replace OLD_SLOT with blue or green)
kubectl patch svc api -n familyroots \
  -p '{"spec":{"selector":{"app":"api","slot":"OLD_SLOT"}}}'
kubectl patch svc frontend -n familyroots \
  -p '{"spec":{"selector":{"app":"frontend","slot":"OLD_SLOT"}}}'

# Scale down the failed new slot
kubectl scale deployment api-NEW_SLOT --replicas=0 -n familyroots
kubectl scale deployment frontend-NEW_SLOT --replicas=0 -n familyroots
...
2. Install prerequisites:
   ...
   ```
3. Apply sealed secrets or re-create `familyroots-secrets`
4. Install the Helm chart:
   ```bash
   helm upgrade --install familyroots ./helm/familyroots \
     --namespace familyroots --create-namespace \
...
| Role | Contact |
|------|---------|
| On-call | PagerDuty — FamilyRoots service |
