...
The refresh-token store is a simple Redis SET with a TTL:
    ...
    Key:   familyroots:refresh:<jti>
    Value: <user_id>
    TTL:   refresh_token_expire_days * 86400

A key-scan on `familyroots:refresh:user:<user_id>:*` handles "logout all
sessions". We use a secondary index:

    Key:   familyroots:refresh:user:<user_id>:<jti>
    Value: 1
    TTL:   same as primary
...
class RedisRefreshTokenRepository(AbstractRefreshTokenRepository):
    ...
    async def revoke_all_for_user(self, user_id: uuid.UUID) -> None:
        ...
        while True:
            ...
            for key in keys:
                # key = familyroots:refresh:user:<uid>:<jti>
                jti = key.split(":")[-1]
                jtis.append(jti)
            if cursor == 0:
                break
