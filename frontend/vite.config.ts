...
export default defineConfig(({ mode }) => {
  ...
  const _hosts = (process.env.VITE_ALLOWED_HOSTS ?? fileEnv.VITE_ALLOWED_HOSTS ?? '').trim();
  ...
  return {
    ...
    server: {
      ...
      proxy: {
        '/api/v1': {
          target: process.env.VITE_API_PROXY_TARGET ?? fileEnv.VITE_API_PROXY_TARGET ?? 'http://localhost:7004',
          changeOrigin: true,
        },
      },
    },
